...
function LinkIcon({ brand }: { brand: boolean }) {
  const cls = 'w-[18px] h-[18px] shrink-0'
  const fill = brand ? 'currentColor' : 'none'
  const stroke = brand ? 'none' : 'currentColor'
  return { cls, fill, stroke }
}

function MailIcon() {
  const { cls, fill, stroke } = LinkIcon({ brand: false })
  return (
    <svg
      className={cls}
      viewBox="0 0 24 24"
...
function GitHubIcon() {
  const { cls, fill, stroke } = LinkIcon({ brand: true })
  return (
    <svg className={cls} viewBox="0 0 24 24" fill={fill} stroke={stroke}>
